...
export const cases = [
  ...
  { name: "kein Einstieg", input: clone((c) => { c.steps[0].type = "task"; }), expectOk: true, expect: ['Kein Schritt hat type "trigger" oder "start"'] },
  { name: "zwei Einstiege", input: clone((c) => { c.steps[1].type = "start"; }), expectOk: true, expect: ["Einstiegspunkte"] },
  { name: "Trigger als Einstieg", input: clone((c) => { c.steps[0].type = "trigger"; }), expectOk: true, expect: ["erfolgreich"] },
  { name: "Trigger auf Deutsch", input: clone((c) => { c.steps[0].type = "Auslöser"; }), expectOk: true, expect: ["gewertet", "trigger"] },
